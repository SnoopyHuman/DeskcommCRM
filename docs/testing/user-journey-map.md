...
| # | Achado | Estado |
|---|---|---|
| 21 | 🔴 **RAG do tenant não existia na prática** (foto de 2026-07-30, não estado). O botão "Configurar" das 4 fontes era stub `disabled` com um toast "Em breve" que, por estar desabilitado, nunca aparecia. Criando a fonte pela API (que funciona, 201), o "Re-indexar" não produzia nada: o handler de `knowledge_source.updated` era stub declarado (S-06.05/06/07); só `nuvemshop.product_synced` indexava de verdade — e a Nuvemshop vem desligada no kit | **fechado em duas etapas — a frase acima já esteve errada duas vezes, então meça, não leia.** A indexação multi-fonte foi resolvida (ver "Prova final, medida" adiante). O cadastro pela tela existe, mas **só nos 2 slots cuja API ingere texto colado** (FAQ e Política): Catálogo e Conversas opt-in não oferecem botão de propósito, porque são preenchidos por pipeline — oferecer obrigava o diálogo a mentir o tipo no envio e colidia no índice único (issue #265). Quais slots têm botão hoje: `grep -n "ingereTextoColado\|comoSePreenche" components/ai/KnowledgeSourceCard.tsx` |
